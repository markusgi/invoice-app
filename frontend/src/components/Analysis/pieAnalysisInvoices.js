...
const AnalysisInvoicePieChart = ({ props }) => {
    ...
    useEffect(() => {
        ...
        const getData = () => {
            props.map((inv) => {
                ...
                data.map((entry) => {
                    return entry.name === inv.shop
                    ? (entry.value += inv.total_amount)
                    : null;
                });
                }
            });
...
export default AnalysisInvoicePieChart;
